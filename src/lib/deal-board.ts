...
// Склонение «сделка/сделки/сделок» по числу (шапка колонки — иначе цифра
// перед суммой нигде не расшифровывается, что это количество сделок).
export function pluralizeDeals(n: number): string {
  const mod10 = n % 10
  const mod100 = n % 100
